function Popup () {
	...
	this.alert = function (title,text) {
		...
		const creatediv = document.createElement('div'); // 创建div
		creatediv.className = 'model';  // 添加class
		creatediv.setAttribute('id','model'); // 添加ID
		const contentHtml = '<div class="model_popup" style="">'
	...
	this.imgConfirm = function (title,text,img,fn) {
		const confirmModel = document.getElementById('imgConfirm');
		if (confirmModel) {
	...
	this.toast = function (text,time) {
		const model = document.getElementById('toast-popup');
		if (model) {
		...
		const contentHtml = '<div class="popup-toast" id="toast-content">' + text + '</div>';
		creatediv.innerHTML = contentHtml;
		document.body.appendChild(creatediv); // 将创建的div 加入 body
		that.cancelToast(time);
	},
